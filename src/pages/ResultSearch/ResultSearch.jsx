...
const ResultSearch = () => {
  const { isLoading, resultSearch } = useSelector(
    (state) => state.resultSearch
  );
  ...
  return (
    <WrapPageSearch>
      <p className="key_search">
        There are{" "}
        <span style={{ color: "yellow" }}>{resultSearch.res?.total}</span>{" "}
            ...
            {[...Array(resultSearch.res?.last_page)].map((item, index) => {
              return (
                <div
                  style={{
                    color: `${
                      resultSearch.res?.current_page === index + 1
                        ? "yellow"
                        : "#ccc"
                    }`,
                  }}
...
const StyledPagination = styled.div`
  ...
  .btn_pre {
    background-color: ${({ status_pre }) =>
      status_pre ? "#000000" : "#3b72fe"};
    cursor: ${({ status_pre }) => (status_pre ? "no-drop" : "pointer")};
  }
  .btn_nex {
    background-color: ${({ status_nex }) =>
      status_nex ? "#000000" : "#3b72fe"};
